...
def prep_metadata():
    blob_path=f"{config['blob_prefix']}/data/metadata/video_metadata.csv"
    ...
    print("Uploading video_metadata to {}".format(blob_path))
    blob = boto3.resource("blob")
    bucket = blob.Bucket(config["blob_bucket"])
    #check if a bucket is exists, if not create a new bucket
    if bucket.creation_date is None:
        blob.create_bucket(Bucket=bucket)
    # Upload video metadata and timestamp
    bucket.upload_file(csv_filepath, blob_path)
    timestamp = blob.Object(config["blob_bucket"], blob_path + ".timestamp")
